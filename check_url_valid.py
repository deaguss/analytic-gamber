...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Contoh penggunaan:
  python video_selector.py --input candidate_urls.json
  python video_selector.py --input urls.json --output hasil.json --target 10
  python video_selector.py --input urls.json --judi-threshold 60 --bukan-threshold 70
  python video_selector.py --input urls.json --max-comments 150 --delay 3
...
Format file input (candidate_urls.json):
  ["https://www.youtube.com/watch?v=AAA", "https://www.youtube.com/watch?v=BBB", ...]
    ...
    parser.add_argument('--input',  '-i', type=Path, default=Config.INPUT_FILE,
                        help='File JSON berisi daftar URL kandidat (default: candidate_urls.json)')
    parser.add_argument('--output', '-o', type=Path, default=Config.OUTPUT_FILE,
                        help='File JSON output hasil seleksi (default: selected_urls.json)')
    ...
    parser.add_argument('--judi-threshold', type=float, default=Config.JUDI_VIDEO_THRESHOLD,
                        help='%% komentar judi agar video = DOMINAN_JUDI (default: 60.0)')
    parser.add_argument('--bukan-threshold', type=float, default=Config.BUKAN_VIDEO_THRESHOLD,
                        help='%% komentar bukan judi agar video = TIDAK_JUDI (default: 70.0)')
